...
@lru_cache(maxsize=1)
def _load_builtin_skill_catalog() -> tuple:
    try:
...
def _coerce_priority(value, default: int = 100) -> int:
    try:
        return int(value)
...
def _normalize_available_ids(available_skill_ids) -> List[str]:
    normalized: List[str] = []
...
def _sort_skill_pool(skills) -> List[object]:
    return sorted(
        skills,
        key=lambda skill: (
...
def _slice_skill_ids(skill_ids: List[str], max_count) -> List[str]:
    if max_count is None:
        return skill_ids
    return skill_ids[:max_count]


def _pick_primary_default_skill_id(candidates) -> str:
    preferred = [
...
def _build_regime_skill_ids(skills) -> Dict[str, List[str]]:
...
DEFAULT_ACTIVE_SKILL_IDS: tuple = tuple(get_default_active_skill_ids())
DEFAULT_ROUTER_SKILL_IDS: tuple = tuple(get_default_router_skill_ids())
...
def extract_skill_id(agent_name) -> Optional[str]:
...
def is_skill_agent_name(agent_name) -> bool:
    return extract_skill_id(agent_name) is not None


def is_skill_consensus_name(agent_name) -> bool:
