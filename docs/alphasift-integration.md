...
- 已确认工作树无未合并文件与未解析 merge marker（`git diff --name-only --diff-filter=U` 无输出，`rg -n "^(<<<<<<< .+|=======$|>>>>>>> .+)"` 无命中），当前结论基于无冲突状态重跑。
- 关键后端契约与安装状态链路（本轮重跑）：
  - `./scripts/ci_gate.sh` 已通过语法检查、flake8 critical checks、`./scripts/test.sh code` 与 `./scripts/test.sh yfinance`；离线全量 pytest 阶段长时间未返回，未形成完整 gate 结论。
  - `timeout 300s python -m pytest tests/test_alphasift_api.py tests/test_system_config_service.py tests/test_llm_channel_config.py tests/test_config_validate_structured.py tests/test_config_env_compat.py tests/test_task_queue_config_sync.py -q`（253 passed）。
- Web 侧兼容性回归：
  - `cd apps/dsa-web && timeout 600s npm ci --ignore-scripts`（通过）。
  - `cd apps/dsa-web && timeout 240s npm run test -- src/api/__tests__/alphasift.test.ts src/hooks/__tests__/useSystemConfig.test.tsx src/pages/__tests__/SettingsPage.test.tsx src/components/layout/__tests__/SidebarNav.test.tsx`（4 files passed，46 tests passed）。
  - `cd apps/dsa-web && timeout 240s npm run lint`（通过）。
  - `cd apps/dsa-web && timeout 300s npm run build`（通过）。
- 与 LLM 配置相关：
