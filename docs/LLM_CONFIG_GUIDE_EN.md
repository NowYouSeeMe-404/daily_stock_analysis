...
- GPT-5 / o-family handling here is a conservative in-repo compatibility strategy for the current dependency window: if you hit cross-provider behavior gaps, first switch to a non-GPT-5 / non-O-family model to verify baseline connectivity and then decide whether to keep the routed path. The project never rewrites your stored `LLM_TEMPERATURE`; only request-time parameters differ.
- Observability and recovery path: request payload adaptation only affects the outgoing LLM call, and `LLM_TEMPERATURE` stays unchanged in saved config. To recover explicit-temperature behavior, reconfigure your chain to avoid GPT-5 / o-family in `LITELLM_MODEL` / `AGENT_LITELLM_MODEL` / `LITELLM_FALLBACK_MODELS` / `LLM_CHANNELS`, or restore a previously exported `.env` through `POST /api/v1/system/config/import`.
- `SystemConfigService` only updates keys that you actually submit when saving from the Web settings page or importing a desktop `.env`; switching to a strict-temperature model does not silently clear, migrate, or rewrite an existing `LLM_TEMPERATURE`. Temporary request-time parameter strategies are not persisted back into the config file. If a provider-specific exception appears later, switch away from that family at runtime first (no migration needed), then adjust `src/llm/generation_params.py` with test-backed regression updates before re-enabling it.
...
- Compatibility evidence and extension path: keep `litellm>=1.80.10,!=1.82.7,!=1.82.8,<2.0.0` as the validation window in `requirements.txt`, and when adjusting rules sync `src/llm/generation_params.py` with `tests/test_llm_channel_config.py` + `tests/test_market_analyzer_generate_text.py` + `tests/test_system_config_service.py` so future maintainers can validate the same evidence.
