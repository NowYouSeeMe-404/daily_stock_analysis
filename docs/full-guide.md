...
`SCHEDULE_TIME` 当前仅用于本地内置调度（`python main.py --schedule`），不会影响 GitHub Actions 的触发时间。本次仅为文档澄清，不是运行时交付；`#1497` 的 Actions 调度能力补齐仍待实现，请改用 `Refs #1497` 跟踪并避免在 PR 上使用 `Closes #1497` 关闭 issue。
...
不能控制触发时间（本轮未实现该能力，文档仅说明边界）。仓库未配置 `SCHEDULE_TIME` 时，默认触发仍按该 workflow 固定 cron（`0 10 * * 1-5`）执行；如需调整 GitHub Actions 触发时刻，请手动修改
...
如需调整 GitHub Actions 的实际触发时间，需要在 fork 或私有仓库中修改
`.github/workflows/00-daily-analysis.yml` 的 cron，并按 UTC 时间换算：
